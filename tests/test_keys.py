...
def test_keypair_generation():
    ...
    assert isinstance(
        key_pair.priv, PrivKey
    ), "Private key is not an instance of PrivKey."
...
def test_deterministic_keypair_generation():
    ...
    assert (
        key_pair1.pub.data == key_pair2.pub.data
    ), "Public keys do not match for the same seed."
    assert (
        key_pair1.priv.data == key_pair2.priv.data
    ), "Private keys do not match for the same seed."
...
def test_keypair_from_private_key():
    ...
    expected_pub_key = (
        ecdsa.SigningKey.from_string(bytes.fromhex(priv_key_str), curve=ecdsa.SECP256k1)
        .get_verifying_key()
        .to_string()
    )
    assert (
        key_pair.pub.data == expected_pub_key
    ), "Public key does not match expected value derived from private key."


def test_pubkey_privkey_hex_representation():
    key_pair = KeyPair.new()
    assert isinstance(
        key_pair.pub.__str__(), str
    ), "Public key string representation is not a string."
    assert isinstance(
        key_pair.priv.__str__(), str
    ), "Private key string representation is not a string."
...
def test_signing_and_validation():
    ...
    assert vk.verify(
        signature, data
    ), "Signature could not be verified with the public key."
